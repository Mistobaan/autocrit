...
from tclx.data.datasets import (RankedDataset, RankedEvalDataset,
                                ranked_data_collator)
...
class RankedTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        for comp in comps:
            ...
            mask = torch.triu(
                torch.full((model.num_reps, model.num_reps), 1, device=diffs.device),
                diagonal=1,
            )
            mask.to(dtype=diffs.dtype)
...
class EvaluateCallback(TrainerCallback):
    def on_epoch_end(self, args, state, control, **kwargs):
        preds = torch.tensor(trainer.predict(trainer.eval_dataset)[0]).view(
            -1, model.num_reps
        )
        bs, num_pairs = preds.shape[0], model.num_reps * (model.num_reps - 1) / 2
        inputs = model.tok.batch_decode(
            [
                trainer.eval_dataset[i][0].flatten().tolist()
                for i in range(model.num_reps * NUM_SAMPLES)
            ]
        )
        num_correct = 0
        ...
        for i, comp in enumerate(preds):
            ...
            mask = torch.triu(
                torch.full((model.num_reps, model.num_reps), 1, device=diffs.device),
                diagonal=1,
            ).to(dtype=diffs.dtype)
            num_correct += torch.sum(mask * diffs > 0)
            # Log samples for wandb
            if i < NUM_SAMPLES:
                for m, response in zip(
                    order, inputs[i * model.num_reps: (i + 1) * model.num_reps]
                ):
...
def train(config):
    ...
    model = make_rm(
        config["model_path"], config["model_type"], config["tokenizer_path"], True
    )
    ...
    order = (
        config["order"] if config.get("order") is not None else ["chosen", "rejected"]
    )
    ...
    eval_dataset = RankedEvalDataset(
        eval_data,
        tokenizer,
        max_length=max_length,
        order=order,
