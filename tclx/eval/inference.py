...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Boolean indicating whether the reward model contains a reference to its underlying causal model",
    )
    ...
    if args.rm_path != "":
        model = load_rm(
            args.model_name, args.tokenizer_name, args.rm_path, args.save_model
        )
        prompt_dataset = []
        for m in args.order:
            prompt_dataset += [
                {
                    "prompt": sample["prompt"] + sample[m] + "<|endoftext|>",
                    "type": m,
                    "id": i,
                }
    ...
    dataloader = torch.utils.data.DataLoader(
        prompt_dataset, batch_size=batch_size, collate_fn=data_collator
    )
