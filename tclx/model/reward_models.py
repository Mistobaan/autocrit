...
class RewardModel(nn.Module):
    def __init__(self, config, PAD_ID, save_model):
        ...
        self.transformer = (
            model.gpt_neox if hasattr(model, "gpt_neox") else model.transformer
        )
        dtype = (
            self.config.torch_dtype
            if hasattr(self.config, "torch_dtype") is not None
            else torch.float32
        )
    ...
    def forward(
        ...
        ends = torch.argmax((input_ids == self.PAD_ID).type(torch.float32), dim=1).view(
            -1, 1
        )
