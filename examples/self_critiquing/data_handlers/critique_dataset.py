        ...
        for obj in reader:
            for item in obj["data"]["questions"]:
                ...
                for answer in item["answers"]:
                    ...
                    if answer["answer"] == "":
                        continue
                    if (answer["feedback"]["rating"] >= 7) and (
                        len(answer["feedback"]["critiques"]) == 0
                    ):
                        critique_data_list.append(
                            {
                                ...
                                "time": obj["time"],
                                "labeler": obj["labeler"],
                                "is_topic_based_summarization": obj[
                                    "is_topic_based_summarization"
                                ],
                                "category": "N/A",
                                "severity": -1,
                                "text_quotes": [],
                ...
                crit_rm_data_list.append(
                    {
                        ...
                        "prompt": f"{start}" f"Question: {item['question']}",
                        "answers": [
                            item["answers"][j]["answer"]
...
if __name__ == "__main__":
    ...
    train_crit, train_ref, train_crit_ref, train_rm_data = extract_data(
        r"train.jsonl(1)\train.jsonl"
    )
    test_crit, test_ref, test_crit_ref, test_rm_data = extract_data(
        r"test.jsonl(1)\test.jsonl"
    )
    DatasetDict(
        {"train": Dataset.from_list(train_crit), "test": Dataset.from_list(test_crit)}
    ).push_to_hub("self-critiquing-critique")
    DatasetDict(
        {"train": Dataset.from_list(train_ref), "test": Dataset.from_list(test_ref)}
    ).push_to_hub("self-critiquing-refine")
    DatasetDict(
        {
            "train": Dataset.from_list(train_crit_ref),
            "test": Dataset.from_list(test_crit_ref),
        }
    ).push_to_hub("self-critiquing-critique-and-refine")
    DatasetDict(
        {
            "train": Dataset.from_list(train_rm_data),
            "test": Dataset.from_list(test_rm_data),
        }
    ).push_to_hub("self-critiquing-critique-answer-ranking")
