...
if __name__ == "__main__":
    ...
    train_sft_data, train_rm_data = extract_helpful_and_rm_data(
        r"train.jsonl(2)\train.jsonl"
    )
    test_sft_data, test_rm_data = extract_helpful_and_rm_data(
        r"test.jsonl(2)\test.jsonl"
    )
    DatasetDict(
        {
    ...
    ).push_to_hub("self-critiquing-helpful-rate")
    DatasetDict(
        {
            "train": Dataset.from_list(train_sft_data),
            "test": Dataset.from_list(test_sft_data),
        }
    ).push_to_hub("self-critiquing-helpful-sft")
